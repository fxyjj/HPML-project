...
if resume:
    ...
    model['W2'] = np.random.randn(int(H/10),H) / np.sqrt(int(H/10)) # Shape will be H
    model['W3'] = np.random.randn(int(H/10)) / np.sqrt(int(H/10)) # Shape will be H
...
def policy_forward(x):
    ...
    h1 = np.dot(model['W1'], x) # (H x D) . (D x 1) = (H x 1) (200 x 1)
    h1[h1<0] = 0 # ReLU introduces non-linearity
    h2 = np.dot(model['W2'], h1) # This is a logits function and outputs a decimal.   (1 x H) . (H x 1) = 1 (scalar)
    h2[h2<0]=0
    logp = np.dot(model['W3'],h2)
    p = sigmoid(logp)  # squashes output to  between 0 & 1 range
    return p, h1,h2 # return probability of taking action 2 (UP), and hidden state
...
def policy_backward(eph1,eph2,epx,epdlogp):
    ...
    dh3 = np.outer(epdlogp, model['W3'])
    dh3[eph2 <= 0] = 0
    dW2 = np.dot(dh3.T, eph1).ravel()
    dh2 = np.outer(eph1, model['W2'])
    dh2[eph1 <= 0] = 0 # backpro prelu
    dW1 = np.dot(dh2.T, epx)
